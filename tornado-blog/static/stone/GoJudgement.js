...
{
	this.BoardSize = boardSize; // 棋盘大小, 默认为19*19
...
GoJudgement.prototype.CountSelfPonnuki = function(i, j)
{
	...
	if ((i-1) < 0)	// 出边
	{
		...
		if ((this.GetColor(i,j) + this.GetColor(i-1,j)) == 0)		// 异色
		{
			this.AroundCountArray[i][j]++;
		} else { // 同色
			if (this.LibertyFlagArray[i-1][j] == true)			// 计算完毕
			{
				this.AroundCountArray[i][j]++;
			} else { // 未计算
				// (i-1,j)无死子
	...
	if ((j-1) < 0)	// 出边
	{
		...
		if ((this.GetColor(i,j) + this.GetColor(i,j-1)) == 0)		// 异色
		{
			this.AroundCountArray[i][j]++;
		} else { // 同色
			if (this.LibertyFlagArray[i][j-1] == true)			// 计算完毕
			{
				this.AroundCountArray[i][j]++;
			} else { // 未计算
				// (i,j-1)无死子
